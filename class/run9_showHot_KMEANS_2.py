...
with open(csv_file_name) as f:
    ...
    for row_count,row in enumerate(rows):
        ...
        if row_count>0 and row_count!=36:
            print(row_count,row[8],row[19],predict_smi(row[8],"model/save-39.pt",str(row_count)+".png"))
...
for i in range(50):
    ...
    if ".pt" in  model_name:
        print(model_name)
        valid(folder+model_name,fig_folder+model_name)

image_list = os.listdir(folder+fig_folder)
create_gif([ folder+fig_folder+'save-'+str(x)+'.pt.png' for x in  range(40)], '1.gif', duration=0.1)
